...
if (isset($_POST['chooselogo']))
{
	...
	if (isset($_POST['source'])) $source = filter_var($_POST['source'], FILTER_SANITIZE_NUMBER_INT);
	if (isset($_POST['departament'])) $departament = filter_var($_POST['departament'], FILTER_SANITIZE_NUMBER_INT);
	if (isset($_POST['flip'])) $flip = filter_var($_POST['flip'], FILTER_SANITIZE_NUMBER_INT);
	...
	if ($flip) $image = flipImage($image, $dimensions[0], $dimensions[1], false, true);

	//  Resize if needed
	if ($dimensions[2]) $logo = resizePng($logo, $dimensions[0], $dimensions[1]);

	// Save stats in database
	if ($image) saveStats($departament, $chooselogo, $source, $flip);
...
{
	$text = $_POST["text"];
	if (isset($_FILES['files']['tmp_name'])) $files = $_FILES['files']['tmp_name'];
	if (isset($_POST['color'])) $color = filter_var($_POST['color'], FILTER_SANITIZE_NUMBER_INT);
	if (isset($_POST['departament'])) $departament = filter_var($_POST['departament'], FILTER_SANITIZE_NUMBER_INT);
	...
	if (count($files) > 1)
	{
		...
		foreach($files as $index => $file)
		{
			...
			//  Resize if needed
			if ($dimensions[2]) $textimage = resizePng($textimage, $dimensions[0], $dimensions[1]);

			// Save stats in database
			if ($image) saveStats($departament, 0, 0, 0, 1, $color);
