...
function checkDimensions($image, $minwidth, $maxwidth, $minheight, $maxheight)
{
	...
	$resize = 0;
	if ($width < $minwidth || $width > $maxwidth || $height < $minheight || $height > $maxheight) handleError(1);
	if (($width !== $maxwidth) && ($height !== $maxheight)) $resize = 1;
	return array(
		$width,
		$height,
		$resize
...
function createText($text, $color, $font, $fontsize, $maxwidth, $maxheight)
{
	...
	// font color
	if ($color) $fontcolor = imagecolorallocate($copyright, 255, 255, 255);
	else $fontcolor = imagecolorallocate($copyright, 0, 0, 0);
